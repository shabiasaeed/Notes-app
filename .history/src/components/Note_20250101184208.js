...
const Note = ({ title, author, time, pinned, onDelete, onPinToggle }) => {
  ...
  return (
    <div className="p-5 rounded-lg bg-white shadow-md hover:shadow-lg flex flex-col gap-2 cursor-pointer w-full max-w-[300px] h-[200px] mx-2 my-2" onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
      <div className={`flex justify-between items-center ${pinned ? '' : 'text-gray-800'}`}>
